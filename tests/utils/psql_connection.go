...
func ForwardPSQLConnection(
	...
	forward, err := PSQLConnectionNew(env, namespace, pod)
	if err != nil {
		return nil, nil, err
	}
	...
	connParameters, err := forward.CreateConnectionParameters(user, pass)
	if err != nil {
		return nil, nil, err
	}

	dbPool := pool.NewPostgresqlConnectionPool(configfile.CreateConnectionString(connParameters))
	conn, err := dbPool.Connection(dbname)
